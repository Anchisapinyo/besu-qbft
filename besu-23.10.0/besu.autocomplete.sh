...
function _complete_besu() {
  ...
  local cmds7=(storage)
  local cmds8=(blocks import)
  local cmds9=(blocks export)
  local cmds10=(public-key export)
  local cmds11=(public-key export-address)
  local cmds12=(password hash)
  local cmds13=(rlp encode)
  local cmds14=(operator generate-blockchain-config)
  local cmds15=(operator generate-log-bloom-cache)
  local cmds16=(operator x-backup-state)
  local cmds17=(operator x-restore-state)
  local cmds18=(storage revert-variables)

  if CompWordsContainsArray "${cmds18[@]}"; then _picocli_besu_storage_revertvariables; return $?; fi
  if CompWordsContainsArray "${cmds17[@]}"; then _picocli_besu_operator_xrestorestate; return $?; fi
  if CompWordsContainsArray "${cmds16[@]}"; then _picocli_besu_operator_xbackupstate; return $?; fi
  if CompWordsContainsArray "${cmds15[@]}"; then _picocli_besu_operator_generatelogbloomcache; return $?; fi
  if CompWordsContainsArray "${cmds14[@]}"; then _picocli_besu_operator_generateblockchainconfig; return $?; fi
  if CompWordsContainsArray "${cmds13[@]}"; then _picocli_besu_rlp_encode; return $?; fi
  if CompWordsContainsArray "${cmds12[@]}"; then _picocli_besu_password_hash; return $?; fi
  if CompWordsContainsArray "${cmds11[@]}"; then _picocli_besu_publickey_exportaddress; return $?; fi
  if CompWordsContainsArray "${cmds10[@]}"; then _picocli_besu_publickey_export; return $?; fi
  if CompWordsContainsArray "${cmds9[@]}"; then _picocli_besu_blocks_export; return $?; fi
  if CompWordsContainsArray "${cmds8[@]}"; then _picocli_besu_blocks_import; return $?; fi
  if CompWordsContainsArray "${cmds7[@]}"; then _picocli_besu_storage; return $?; fi
...
function _picocli_besu_storage() {
  ...
  local commands="revert-variables"
  local flag_opts="-h --help -V --version"
  local arg_opts=""
